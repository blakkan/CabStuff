...
With the external tables complete, the transformation begins. The records are aggregated into a single table for yellow and green cabs. This work is represented in the file load_rides_yg.py.  We aggregate to ride source, latitude/longitude, pickup hour/day/month/year, and passenger count. We sum the total rides for against each distinct key. In this python script, we are running the transformation by ride_source and month. We also split the record selection (select statement) and insert into the master table into separate steps. This was for performance reasons. Alternative to this python approach, 
